...
public class ServerContainer {
        ...
        static {
                final Pair<ServerContainer, ModConfigSpec> specPair = new ModConfigSpec.Builder()
                                .configure(ServerContainer::new);
        ...
        ServerContainer(ModConfigSpec.Builder b) {
                ...
                SKULL_HIDES_LEVEL = b.comment("Mobs that are much higher level will hide their levels.")
                                .define("SKULL_HIDES_LEVEL", true);
                REMOVE_ATK_SPEED_COOLDOWN = b
                                .comment("Mns tries to replicate vanilla atk speed and reduces dmg of basic attacks if hit too often")
                                .define("REMOVE_ATK_SPEED_COOLDOWN", false);
                REMOVE_DRAW_SPEED_COOLDOWN = b.comment(
                                "Mns tries to replicate vanilla bow draw and reduces dmg of basic attacks if bow is not fully drawn. Enable for machine gun bows.")
                                .define("REMOVE_DRAW_SPEED_COOLDOWN", false);
                ...
                LEVEL_DISTANCE_PENALTY_LEEWAY = b.defineInRange("LEVEL_DISTANCE_PENALTY_LEEWAY", 2, 0,
                                Integer.MAX_VALUE);
                ...
                LEVEL_DISTANCE_SKULL_SHOW = b
                                .comment("When mobs are this many levels higher than you, a skull will show, indicating danger.")
                                .defineInRange("SHOW_SKULL_ON_MOBS_THAT_ARE_X_LEVELS_HIGHER_THAN_YOU", 5, 1, 1000);
                LEVEL_DISTANCE_PENALTY_PER_LVL = b.comment(
                                "When you are higher or lower level than mobs by X levels, decreases the loot and exp droprate.")
                                .defineInRange("lvl_distance_loot_penalty_per_level", 0.2D, 0, 1D);
                ...
                MOB_DESPAWN_DISTANCE_IN_MAPS = b
                                .comment("Distance needed from player to despawn mobs in maps, lower means more despawning")
                ...
                COMBAT_TO_PROFESSION_RESTED_XP_GENERATION = b.defineInRange("COMBAT_TO_PROFESSION_RESTED_XP_GENERATION",
                                0.25,
                                0, 1);
                PROFESSION_TO_COMBAT_RESTED_XP_GENERATION = b.defineInRange("PROFESSION_TO_COMBAT_RESTED_XP_GENERATION",
                                0.1, 0,
                                1);
                RESTED_XP_DEATH_PENALTY = b.defineInRange("RESTED_XP_DEATH_PENALTY", 0.5F, 0, 1);
                PARTY_EXP_BONUS = b.comment("Additional exp before splitting between players.").defineInRange(
                                "PARTY_EXP_BONUS",
                                0.2F, 0, 1);
                PARTY_DROP_BONUS = b.comment("Additional drop before splitting between players.")
                                .defineInRange("PARTY_DROP_BONUS", 0.2F, 0, 1);
                ...
                // DONT_MAKE_MAP_MOBS_PERSISTENT_IF_MOB_COUNT_IS_ABOVE =
                // b.defineInRange("DONT_MAKE_MAP_MOBS_PERSISTENT_IF_MOB_COUNT_IS_ABOVE", 25, 0,
                // 10000);
                ...
                BONUS_EXP_PERCENT_PER_HIGHER_LVL_CHARACTERS = b.defineInRange(
                ...
                ENTITY_SUMMON_BLACKLIST = b.comment("Entities that summons should not attack")
                                .defineList("ENTITY_SUMMON_BLACKLIST", Arrays.asList(
                                                "minecraft:allay",
                                                "minecraft:armor_stand"), x -> x instanceof String);

                b.comment("These are just default values for the Mine and slash Hub > features")
                                .push("Default Feature Configs");
                                        ...
                                        .forEach(x -> {
                                                try {
                                                        ...
                                                        ResourceLocation id = ResourceLocation
                                                                        .fromNamespaceAndPath(array[0], array[1]);
                                                        Item item = BuiltInRegistries.ITEM.get(id);

                                                        if (item != Items.AIR && item != null) {
                                                                if (!ExileDB.GearSlots().isRegistered(array[2])) {
                                                                        cachedCompatMap.put(item, null); // if invalid,
                                                                                                         // make the
                                                                                                         // item have no
                                                                                                         // gear type,
                                                                                                         // this was
        ...
        public boolean isEntitySummonBlacklisted(LivingEntity entity) {
                String id = Objects.requireNonNull(BuiltInRegistries.ENTITY_TYPE.getKey(entity.getType())).toString();
