...
public class SuppGemOverlayButton extends ImageButton {
    ...
    public SuppGemOverlayButton(boolean can, MaxLinks links, int xPos, int yPos) {
        super(xPos, yPos, BUTTON_SIZE_X, BUTTON_SIZE_Y,
                new WidgetSprites(SlashRef.guiId("blocked_slot"), SlashRef.guiId("blocked_slot")),
                (button) -> {
                    // Minecraft.getInstance().setScreen(new
                    // InvGuiScreen(GuiInventoryGrids.ofSelectableSpells(ClientOnly.getPlayer(),
                    // slot)));
                });
        this.can = can;
        this.links = links;
    ...
    public void renderWidget(GuiGraphics gui, int mouseX, int mouseY, float delta) {
        ...
        if (!can) {
            ...
            gui.blit(id, getX(), getY(), BUTTON_SIZE_X, BUTTON_SIZE_X, BUTTON_SIZE_X, BUTTON_SIZE_X, BUTTON_SIZE_X,
                    BUTTON_SIZE_X);
        }
    }
