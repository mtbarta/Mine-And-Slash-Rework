...
public class BackpackQuickLootButton extends ImageButton {
    ...
    public BackpackQuickLootButton(int pX, int pY, ContainerScreen screen) {
        super(pX + ClientConfigs.getConfig().QUICK_LOOT_BUTTON_X_OFFSET.get(),
                pY + ClientConfigs.getConfig().QUICK_LOOT_BUTTON_Y_OFFSET.get(),
                14, 14,
                new WidgetSprites(TEXTURE, TEXTURE),
                x -> Packets.sendToServer(new BackPackLootMenuPacket(
                        Screen.hasShiftDown() ? BackPackLootMenuPacket.Mode.DROP : BackPackLootMenuPacket.Mode.LOOT)),
    ...
    public static void addLootButton(ScreenEvent.Init event) {
        ...
        if (Minecraft.getInstance().screen instanceof ContainerScreen containerScreen
                && ClientConfigs.getConfig().ENABLE_QUICK_LOOT_BUTTON.get()) {
            event.addListener(new BackpackQuickLootButton(containerScreen.getGuiLeft() + containerScreen.getXSize(),
                    containerScreen.getGuiTop(), containerScreen));
        }
    }
}
