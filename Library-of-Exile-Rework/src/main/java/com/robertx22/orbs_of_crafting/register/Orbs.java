...
public class Orbs extends ExileKeyHolder<ExileCurrency> {
    ...
    // uh, how do i make sure the order of class instantiations is correct across
    // mods..?

    public static Orbs INSTANCE = (Orbs) new Orbs(Ref.REGISTER_INFO)
            .itemIds(new ItemIdProvider(x -> OrbsRef.id(x)))
            .createItems(new ItemCreator<ExileCurrency>(x -> new Item(new Item.Properties().stacksTo(64))),
                    x -> RegObj.register(x.itemID(), x.item(), CommonInit.ITEMS));

    public Orbs(ModRequiredRegisterInfo modRegisterInfo) {
        super(modRegisterInfo);
    }

    public ExileKey<ExileCurrency, IdKey> LEGENDARY_TOOL_ENCHANT = ExileCurrency.Builder
            .of("legendary_enchant_tool", "Orb of Legendary Tool Enchant")
